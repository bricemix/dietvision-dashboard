module AdminHelper
  ...
  def stat_card(label, value, icon_svg, icon_color = "text-gray-400")
    content_tag :div, class: "bg-gray-900 rounded-xl border border-gray-800 p-5" do
      header = content_tag(:div, class: "flex items-center justify-between mb-3") do
        label_el = content_tag(:div, label, class: "text-xs text-gray-500 font-medium uppercase tracking-wide")
        icon_el  = content_tag(:span, icon_svg.html_safe, class: icon_color)
        label_el + icon_el
      end
      value_el = content_tag(:div, value.to_s, class: "text-2xl font-bold text-white")
      header + value_el
    end
  end
  ...
  def icon_x_circle
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>'
  end

  def icon_trend_up
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>'
  end
  ...
  def icon_refresh
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>'
  end
  ...
  def icon_flask
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3h6M9 3v8l-4 9h14l-4-9V3"/><path d="M6 18h12"/></svg>'
  end
  ...
  # Surligne un mot-clé dans un texte (pour les logs)
  def highlight_keywords(text, keyword)
    return h(text) if keyword.blank?
    escaped_text = h(text)
    pattern = Regexp.escape(CGI.escapeHTML(keyword))
    highlighted = escaped_text.gsub(
      /#{pattern}/i,
      '<mark style="background:#c8ff0033;color:#c8ff00;border-radius:2px;padding:0 2px">\0</mark>'
    )
